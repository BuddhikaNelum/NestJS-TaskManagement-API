...
export class TasksController {
    ...
    getTasks(@Query(ValidationPipe) filterDto: GetTaskFilterDto, @GetUser() user: User) {
        this.logger.verbose(`User "${user.username}" retrieving from tasks. Filters ${JSON.stringify(filterDto)}`)
    ...
    createTask(@Body() createTaskDto: CreateTaskDto, @GetUser() user: User): Promise<Task> {
        this.logger.verbose(`User "${user.username}" crates a task. Task ${JSON.stringify(createTaskDto)}`)
